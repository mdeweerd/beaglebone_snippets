#!/usr/bin/env python3
# shutdown the system when we are on battery power for a while
# Warning: not currently adapted for ASC.
...
def ledOn(n=1):
    ...
    os.system(
        "echo 255 > /sys/class/leds/beaglebone:green:usr%d/brightness" % n
    )
...
def fixLEDs():
    ...
    for t in triggers:
        os.system(
            "echo %s > /sys/class/leds/beaglebone:green:usr%d/trigger" % (t, n)
        )
        n += 1
    ...
    def run(self):
        ...
        while True:
            ...
            if charging != tmpCharging:
                msg = (
                    "Started Charging Battery"
                    if tmpCharging
                    else "Stopped Charging Battery"
                )
                my_logger.info(msg)
            ...
            if onBattery:
                ...
                if timeOnBattery > BATT_SHUTDOWN_S:
                    my_logger.info(
                        "On Battery power for %d secs - shutdown NOW!"
                        % timeOnBattery
                    )
                    ...
                    int(timeOnBattery) > lastNotifySecond
                    and int(timeOnBattery) % notifyMod == 0
                ):
                    lastNotifySecond = int(timeOnBattery)
                    my_logger.info(
                        "On Battery Power -- auto-shutdown in %d seconds"
                        % (BATT_SHUTDOWN_S - int(timeOnBattery))
                    )
                time.sleep(0.5)
            else:
                timeOnBattery = 0
